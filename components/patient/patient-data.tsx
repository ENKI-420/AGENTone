...
import { type FHIRPatient, fhirService } from "@/lib/fhir-service"
import { type GenomicReport, genomicService } from "@/lib/genomic-service"
import { securityService } from "@/lib/security-service"
import { useAuth } from "@/components/auth/auth-provider"
import { useToast } from "@/hooks/use-toast"
import { IconDna, IconUser, IconFileReport, IconAlertTriangle } from "@tabler/icons-react"
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      try {
        ...
        // Fetch patient data
        const patientData = await fhirService.getPatient(patientId)
        if (!patientData) {
        ...
        // Fetch genomic data
        const report = await genomicService.processGenomicData(patientId)
        setGenomicReport(report)
      } catch (err: any) {
    ...
    return (
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>Patient information could not be found</CardDescription>
        </CardHeader>
      </Card>
    )
  }
          ...
          <CardContent>
            {genomicReport ? (
              ...
              <div className="text-center py-8">
                ...
                <Button className="mt-4">Upload Genomic Data</Button>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center py-8">
              ...
              <Button className="mt-4">Upload Reports</Button>
            </div>
          </CardContent>
        </Card>
      </TabsContent>
